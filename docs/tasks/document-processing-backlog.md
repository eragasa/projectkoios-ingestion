...
`koios-compose-pdf-transcripts-batch` is dry-run by default and consumes the existing hash-locked batch plan after raw extraction and equation detection. Explicit `--apply` replays equation evidence, derives layouts, article structure, tables, figures, and complete structured transcription, projects clean text, and requires a passing derivation audit that includes the clean projection. The implemented ING-TRANSCRIPT-02 baseline published immutable `derived/transcription/clean.json`, `clean.txt`, `audit.json`, and `manifest.json` as owner-internal transcript batch manifest schema `1`. ING-REFERENCE-EVIDENCE-01 extends the current command under owner-internal schema `2` with `reference-evidence.json`; this batch-manifest generation is distinct from clean-transcript artifact generation `1` and Proposed reference-evidence schema generation `1`. Schema `2` binds its version, reconstructible intermediate result identities, and all five persisted hashes while avoiding multi-hundred-megabyte duplication of complete table, figure, and transcription graphs. Replay is byte-identical. Legacy four-file schema-`1` sets remain preserved and fail closed as incomplete rather than being relabeled or repaired; other incomplete, unsafe, or different existing artifacts also fail closed. This extension neither implements transcript v2 nor accepts a cross-repository contract.
